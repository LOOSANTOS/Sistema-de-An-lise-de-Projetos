...
<body>
        ...
        <div class="modal-content">
            ...
            <div style="margin-bottom: 20px;">
                <label style="display: flex; align-items: center; cursor: pointer;">
                    <input type="checkbox" id="includeDescription" checked style="margin-right: 10px;">
                    Incluir descrição dos projetos
                </label>
            </div>
            <div style="display: flex; justify-content: center; margin-top: 20px;">
                <button onclick="generatePdfWithOptions()">Gerar PDF</button>
                <button onclick="document.getElementById('pdfConfigModal').style.display = 'none'" 
                        style="background: #f0f2f5; color: #333; margin-left: 15px;">
                    Cancelar
                </button>
            </div>
        </div>
    ...
    <script>
        ...
        async function generatePdfWithOptions() {
            ...
            const tableColumn = includeDescription 
                ? ["Nº Processo", "Descrição", "Data", "Status"]
                : ["Nº Processo", "Data", "Status"];

            const tableRows = projects.map(p => {
                return includeDescription
                    ? [p.name, p.description, p.date, p.status || 'Pendente']
                    : [p.name, p.date, p.status || 'Pendente'];
            });
            ...
            doc.autoTable({
                ...
                columnStyles: {
                    0: { cellWidth: 30 },
                    1: { cellWidth: includeDescription ? 'auto' : 25 },
                    ...(includeDescription && { 
                        2: { cellWidth: 25 },
                        3: { cellWidth: 25 }
                    })
